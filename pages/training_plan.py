...
from helpers.training_page_functions import empty_zones,get_zone_minutes_from_steps,get_completed_week_zones,get_workout_duration,get_workout_zone_minutes,format_total_time,format_minutes,clean_activity_type,render_small_progress_circle,render_clickable_workout_card,render_week_summary,render_full_week,zone_color,inject_training_page_css
...
# BOX 1 START: WEEKLY AVAILABILITY + TRAINING DAYS
st.markdown('<div class="planning-section">',unsafe_allow_html=True)

availability_col,training_days_col=st.columns([3,1],gap="large")
...
with availability_col:
    ...
    with header_col2:
        if st.button("Edit",key="edit_weekly_availability"):
            st.session_state["profile_section"]="Weekly Availability"
            st.switch_page("pages/profile.py")
    ...
    for col,day in zip(day_cols,day_order):
        data=weekly_availability.get(day,{})
        available=isinstance(data,dict) and (float(data.get("hours",0) or 0)>0 or (data.get("available") and data.get("start") and data.get("end")))
        hours=float(data.get("hours",0) or 0) if isinstance(data,dict) else 0

        if available:
            time_text=f"{hours:g}h" if hours>0 else f"{data.get('start')}–{data.get('end')}" if data.get("start") and data.get("end") else "Available"
            card_bg="#f4f8fb"
            border="#d8e2e9"
            day_color="#17212b"
            value_color="#526170"
        ...
        with col:
            st.markdown(f'<div style="background:{card_bg};border:1px solid {border};border-radius:7px;padding:5px 2px;text-align:center;min-height:48px;display:flex;flex-direction:column;justify-content:center;"><div style="font-size:8px;font-weight:700;letter-spacing:.04em;color:{day_color};">{day_short[day]}</div><div style="font-size:9px;font-weight:600;color:{value_color};margin-top:3px;white-space:nowrap;">{time_text}</div></div>',unsafe_allow_html=True)

with training_days_col:
    st.markdown("**Training Days**")
    st.markdown('<div style="font-size:9px;color:#8a96a3;margin-top:-4px;margin-bottom:4px;">Sessions per week</div>',unsafe_allow_html=True)

    sessions_per_week=st.segmented_control("Sessions per week",options=session_count_options,selection_mode="single",default=default_sessions,key="sessions_per_week_select",label_visibility="collapsed")
...
workouts=load_workouts(LIBRARY_PATH)
...
# BOX 2 START: TRAINING PLAN HEADER
st.markdown('<div class="planning-section">',unsafe_allow_html=True)

header_col,edit_col=st.columns([8,1],vertical_alignment="center")

with header_col:
    st.markdown(f'<div style="font-size:16px;font-weight:700;color:#17212b;">Training Plan <span style="font-size:13px;color:#94A3B8;font-weight:400;">for {goal_label} · {sport}</span></div>',unsafe_allow_html=True)
...
if previous_week_tss>0:
    ...
    st.markdown(f'<div style="font-size:9px;color:#8a96a3;margin-top:2px;">Last week: {previous_week_tss:.0f} TSS · This week: {weekly_tss:.0f} TSS · {change:+.0f}%</div>',unsafe_allow_html=True)
else:
    st.markdown('<div style="font-size:9px;color:#8a96a3;margin-top:2px;">Build your week and choose the workout that fits you best.</div>',unsafe_allow_html=True)
...
for column,current_date in zip(timeline_cols,timeline_dates):
    with column:
        ...
        if not is_past and plan and plan.get("workout"):
            render_clickable_workout_card(plan["workout"],current_date.strftime("%A"),short_date,activity_name,detail,color,f"timeline_{current_date}")
        else:
            st.markdown(f'<div style="background:{color}18;border:1px solid {color}55;border-top:3px solid {color};border-radius:7px;padding:7px 4px;min-height:64px;text-align:center;"><div style="font-size:8px;color:#7a8792;font-weight:700;">{date_label}{" · TODAY" if current_date==today else ""}</div><div style="font-size:10px;color:#17212b;font-weight:700;margin-top:8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{activity_name}</div><div style="font-size:8px;color:#6b7785;margin-top:3px;">{detail}</div></div>',unsafe_allow_html=True)
...
# BOX 4 START: WEEKLY TRAINING LOAD
st.markdown('<div class="planning-section">',unsafe_allow_html=True)
...
for column,zone in zip(circle_cols[:4],display_zones):
    with column:
        ...
        render_small_progress_circle(zone,percentage,f"{format_total_time(completed)} / {format_total_time(target)}",zone_color(zone))
...
# BOX 5 START: FULL PLAN UNTIL GOAL
st.markdown('<div class="planning-section">',unsafe_allow_html=True)
...
for week_number,week_plans in weeks.items():
    ...
    with summary_col:
        render_week_summary(week_plans,zone_target,tss_target)

    with plan_col:
        render_full_week(week_plans,username)
