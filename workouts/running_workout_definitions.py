...
class Workout:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        data.update({"sport": self.sport,"duration_minutes": round(self.duration_seconds / 60),"estimated_tss": self.target_tss,"target_tss": self.target_tss,"target_if": round(self.target_if, 2),"interval_count": sum(step.repeat for step in self.steps if "interval" in step.name.lower() or "work" in step.name.lower()),"interval_duration": next((step.duration_seconds for step in self.steps if "work" in step.name.lower()), 0),"recovery_duration": next((step.duration_seconds for step in self.steps if "recover" in step.name.lower()), 0),"sets": max((step.repeat for step in self.steps), default=1)})
        return data
...
def warmup(minutes=5):
    return Step("Warm-up", minutes * 60, 65, description="Easy running with gradual progression.")

def cooldown(minutes=5):
    return Step("Cool-down", minutes * 60, 60, description="Easy running to bring the effort down.")

def recovery(minutes, intensity=60):
...
def strides(count=4, duration_seconds=20, recovery_seconds=60):
    ...
    for i in range(count):
        ...
        if i < count - 1:
            steps.append(Step(f"Stride recovery {i + 1}", recovery_seconds, 60))
    return steps
def generate_running_workouts():
    workouts = []
    easy_families = {"recovery_run": (30, 58),"easy_run": (45, 65),"aerobic_run": (55, 70),"progressive_easy": (50, 68)}
    for family, (base_minutes, intensity) in easy_families.items():
        for variant in range(1, 7):
            ...
            if family == "progressive_easy":
                half = duration // 2
                steps = [work(half, intensity - 3, "Easy running"),work(duration - half, intensity + 5, "Progressive finish")]
            else:
    ...
    long_families = {"long_run": (70, 68),"progressive_long_run": (75, 68),"long_run_finish": (80, 68)}
    for family, (base_minutes, intensity) in long_families.items():
        for variant in range(1, 7):
            duration = base_minutes + variant * 8
            if family == "progressive_long_run":
                steps = [work(duration - 20, intensity, "Long aerobic block"),work(20, 76 + variant % 3, "Progressive finish")]
            elif family == "long_run_finish":
                steps = [work(duration - 25, intensity, "Long aerobic block"),work(15, 76, "Tempo finish"),work(10, 82 + variant % 2, "Strong finish")]
            else:
    ...
    tempo_families = {"steady_tempo": (10, 82),"tempo_blocks": (12, 84),"progressive_tempo": (10, 80),"tempo_ladder": (6, 82),"tempo_endurance": (15, 80)}
    ...
    threshold_families = {"cruise_intervals": (3, 8, 3, 96),"threshold_intervals": (4, 6, 3, 100),"long_threshold": (3, 10, 3, 98),"threshold_ladder": (3, 5, 2, 98),"broken_threshold": (2, 15, 4, 96),"sustained_threshold": (1, 25, 0, 95)}
    ...
    for family, (work_seconds, recover_seconds, intensity) in ronnestad.items():
        for variant in range(1, 7):
            ...
            for s in range(current_sets):
                for r in range(13):
                    steps.append(Step(f"Ronnestad 30 sec Work {s + 1}.{r + 1}", work_seconds, intensity + variant % 3))
                    if r < 12:
                        steps.append(Step(f"Ronnestad 15 sec Recovery {s + 1}.{r + 1}", recover_seconds, 62))
                if s < current_sets - 1:
                    steps.append(recovery(3, 60))
            workouts.append(build_workout(family, variant, "VO2max", family, steps, ["ronnestad", "30-15", "vo2max", "high-intensity"], 5, 5))
    vo2_families = {"four_by_four": (4, 4, 3, 108),"five_by_three": (5, 3, 2, 110),"six_by_two": (6, 2, 2, 112),"short_vo2": (10, 1, 1, 118),"long_vo2": (4, 5, 3, 108),"vo2_ladder": (5, 2, 2, 106)}
